  ...
  def _parse_seekhead(self, seek_head, segment, stream, specs):
    for seek in seek_head:
      ...
      if element_name == "Info":
        ...
        self.info = Info.fromelement(ebml.parse_element(stream, specs, True, ignore_element_names=ignores))
      elif element_name == "Tracks":
        stream.seek(element_position)
        tracks = ebml.parse_element(stream, specs, True, ignore_element_names=ignores)
        self.video_tracks.extend([VideoTrack.fromelement(t) for t in tracks if t["TrackType"].data == VIDEO_TRACK])
      elif element_name == "Tags":
        stream.seek(element_position)
        self.tags.extend([Tag.fromelement(t) for t in ebml.parse_element(stream, specs, True, ignore_element_names=ignores)])
      elif element_name == "Cues":
        stream.seek(element_position)
        self.cues.extend([Cue.fromelement(t) for t in ebml.parse_element(stream, specs, True, ignore_element_names=ignores)])
      elif element_name == "SeekHead" and self.recurse_seek_head:
        stream.seek(element_position)
        self._parse_seekhead(ebml.parse_element(stream, specs, True, ignore_element_names=ignores), segment, stream, specs)
...
if __name__ == "__main__":
  with open(sys.argv[1], "rb") as file:
    ...
    track_frames = get_track_frames(mkv, track)
    ...
    frames = [str(round(timecode_scale / frame_duration * t)) for t in timestamps]

    print(",".join(frames))
    print("total frames:", track_frames)
